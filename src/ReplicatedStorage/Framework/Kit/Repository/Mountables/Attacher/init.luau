...
function Attacher.Run(scope: _T.Scope, utility: _T.Utility)
	...
	local function attach(hitPart)
		...
		local isPlayer = hitPart.Parent == character
		local attachTo = nil

		if isPlayer and table.find(utility.Character.getHitbox("WholeBody"), hitPart) then
	...
	if touchConfiguration.canFlip then
		scope:add(utility.ClientObjects.bindToFlip(trigger, attach))
	end
